...
set_user(char *username)
{
	...
	if (group) {
		...
		if (!gr) {
			syslog(LOG_ERR, "group '%s' not found", user);
			return NULL;
		}
	}
	...
	if (setgid(gr ? gr->gr_gid : pw->pw_gid)) {
		syslog(LOG_ERR, "unable to set groupid to %d", gr ? gr->gr_gid : pw->pw_gid);
		return NULL;
	}
	...
	if (setuid(pw->pw_uid)) {
#endif
		syslog(LOG_ERR, "unable to set userid to %d", pw->pw_uid);
		return NULL;
	}
...
static pid_t
spawn_child(int respawn_count, struct notify *notify, const char *svcname, char **argv)
{
	pid_t child_pid;
	...
	case 0:
		child_process(svcname, notify, argv);
	case -1:
...
supervise(const char *svcname)
{
	...
	struct pollfd pfds[] = {
		[CONTROL] = { .fd = setup_control(), .events = POLLIN },
		[NOTIFY] = { .fd = setup_notify(svcname), .events = POLLIN },
		[CHILD] = { .events = POLLIN },
		[READY] = { .events = POLLIN },
	};
	...
	child_pid = spawn_child(respawn_count, NULL, svcname, argv);
