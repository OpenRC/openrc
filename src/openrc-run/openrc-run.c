...
int main(int argc, char **argv)
{
	...
	scriptdir = dirname(service_path);
	initdir = strrchr(scriptdir, '/');
	if (!initdir || strcmp(initdir, "/init.d") != 0) {
		fprintf(stderr, "%s is not an init dir", service_path);
		exit(EXIT_FAILURE);
	}
	*initdir = '\0';
	...
	setenv("RC_SCRIPTDIR", scriptdir, true);
