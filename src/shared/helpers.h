...
RC_UNUSED static FILE *fopenat(int dirfd, const char *pathname, int mode)
{
	int fd = openat(dirfd, pathname, mode);
	const char *fmode;
	FILE *fp;
...
RC_UNUSED static DIR *opendirat(int dirfd, const char *pathname)
{
...
RC_UNUSED static DIR *opendir_dup(int dirfd)
{
	int fd = dup(dirfd);
	DIR *dp;
