...
#define _GNU_SOURCE /* https://github.com/OpenRC/openrc/pull/801 */
#include <errno.h>
#include <fcntl.h>
#include <signal.h>
#include <sys/wait.h>
...
rc_exec(struct rc_exec_args *args)
{
	...
	if (pipe2(execpipe, O_CLOEXEC) < 0)
		goto exit;
	if (args->redirect_stdin == RC_EXEC_MKPIPE) {
		if (pipe2(stdin_pipe, O_CLOEXEC) < 0)
			goto exit;
		args->redirect_stdin = stdin_pipe[0];
	}
	if (args->redirect_stdout == RC_EXEC_MKPIPE) {
		if (pipe2(stdout_pipe, O_CLOEXEC) < 0)
			goto exit;
		args->redirect_stdout = stdout_pipe[1];
	}
	if (args->redirect_stderr == RC_EXEC_MKPIPE) {
	...
	if (res.pid < 0) {
		...
		/* child */
		if (args->redirect_stdin != RC_EXEC_REDIRECT_NONE &&
		    dup2(args->redirect_stdin, STDIN_FILENO) < 0)
			goto child_err;
		if (args->redirect_stdout != RC_EXEC_REDIRECT_NONE &&
		    dup2(args->redirect_stdout, STDOUT_FILENO) < 0)
			goto child_err;
		if (args->redirect_stderr != RC_EXEC_REDIRECT_NONE &&
