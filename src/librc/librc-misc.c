...
{
	return config_list(fopen(file, "r"));
}
...
rc_config_directory(RC_STRINGLIST *config, int targetfd, const char *dir)
{
	...
	TAILQ_FOREACH(fname, rc_conf_d_files, entries) {
		...
		rc_conf_d_list = config_list(fopenat(dirfd(dp), fname->value, O_RDONLY));
...
{
	return config_load(fopen(file, "r"));
}
...
{
	FILE *fp = fopenat(rc_dirfd(dir), "rc.conf", O_RDONLY);
	RC_STRINGLIST *conf = config_load(fp);
...
rc_conf_value(const char *setting)
{
	...
	if (exists(RC_CONF_OLD)) {
		RC_STRINGLIST *old_conf = config_load(fopen(RC_CONF_OLD, "r"));
