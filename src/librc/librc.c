...
rm_dir(int targetfd, const char *pathname, bool top)
{
	...
	if (!(dp = opendirat(targetfd, pathname)))
		return false;
...
rc_runlevel_get(void)
{
	...
	if ((fp = fopenat(rc_dirfd(RC_DIR_SVCDIR), "softlevel", O_RDONLY))) {
...
rc_runlevel_set(const char *runlevel)
{
	...
	if (!(fp = fopenat(rc_dirfd(RC_DIR_SVCDIR), "softlevel", O_WRONLY | O_CREAT | O_TRUNC)))
		return false;
...
rc_service_mark(const char *service, const RC_SERVICE state)
{
	...
	if (state == RC_SERVICE_STARTED || state == RC_SERVICE_STOPPED) {
		DIR *dp = opendir_dup(rc_dirfd(RC_DIR_SCHEDULED));
		if (dp) {
			struct dirent *d;
			while ((d = readdir(dp))) {
				char *was;
...
rc_service_value_set(const char *service, const char *option, const char *value)
{
	...
	} else if ((fp = fopenat(optdirfd, option, O_WRONLY | O_CREAT | O_TRUNC))) {
		fprintf(fp, "%s", value);
		fclose(fp);
	} else {
		ret = false;
...
rc_services_in_state(RC_SERVICE state)
{
	...
	list = rc_stringlist_new();
	if (!(dp = opendirat(rc_dirfd(RC_DIR_SVCDIR), "scheduled")))
		return list;
...
rc_services_scheduled_by(const char *service)
{
	...
	if (!(dp = opendir_dup(rc_dirfd(RC_DIR_SCHEDULED))))
		return list;

	/* TODO: should be the full filename */
	while ((d = readdir(dp))) {
		char *file;
