...
{
	return deptree_load_file(fopenat(rc_dirfd(RC_DIR_SVCDIR), "deptree", O_RDONLY));
}

RC_DEPTREE *
rc_deptree_load_file(const char *deptree_file)
{
	return deptree_load_file(fopen(deptree_file, "r"));
}
...
deep_mtime_check(int target_dir, const char *target, bool newer, time_t *rel, char *file)
{
	...
	if (!S_ISDIR(buf.st_mode) || !(dp = opendirat(target_dir, target)))
		return retval;
...
rc_deptree_update_needed(time_t *newest, char *file)
{
	...
	config = config_list(fopenat(rc_dirfd(RC_DIR_SVCDIR), "depconfig", O_RDONLY));
