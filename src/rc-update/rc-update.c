...
const struct option longopts[] = {{"all", 0, NULL, 'a'},
				  {"stack", 0, NULL, 's'},
				  {"update", 0, NULL, 'u'},
				  longopts_COMMON};
const char *const longopts_help[] = {"Process all runlevels",
...
static int add(const char *runlevel, const char *service)
{
	...
	if (!rc_service_exists(service)) {
		if (errno == ENOEXEC)
			eerror("%s: service `%s' is not executable", applet,
			       service);
		else
			eerror("%s: service `%s' does not exist", applet,
			       service);
...
static int delete (const char *runlevel, const char *service)
{
	...
	if (rc_service_delete(runlevel, service)) {
		einfo("service %s removed from runlevel %s", service, runlevel);
		return 1;
	}

	if (errno == ENOENT)
		eerror("%s: service `%s' is not in the runlevel `%s'", applet,
		       service, runlevel);
	else
...
static int addstack(const char *runlevel, const char *stack)
{
	...
	if (strcmp(runlevel, RC_LEVEL_SYSINIT) == 0 ||
		...
		eerror("%s: cannot stack the %s runlevel", applet,
		       RC_LEVEL_SYSINIT);
		return -1;
	}
	if (!rc_runlevel_stack(runlevel, stack)) {
		eerror("%s: failed to stack `%s' to `%s': %s", applet, stack,
		       runlevel, strerror(errno));
		return -1;
	}
...
static int delstack(const char *runlevel, const char *stack)
{
	...
	if (errno == ENOENT)
		eerror("%s: runlevel `%s' is not in the runlevel `%s'", applet,
		       stack, runlevel);
	else
...
int main(int argc, char **argv)
{
	...
	if (optind < argc) {
		...
		if (action)
			...
			eerrorx("%s: invalid command `%s'", applet,
				argv[optind]);
	}
	if (!action)
		action = DOSHOW;
	...
	if (action & DOSHOW) {
		...
		if (!service)
			...
			if (action & DOADD) {
				if (all_runlevels) {
					rc_stringlist_free(runlevels);
					eerrorx("%s: the -a option is invalid with add",
						applet);
				}
